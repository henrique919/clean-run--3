  ...
  return (
    <SectionCard title="Subcontractor response" icon={Send}>
      {item.rectificationEvidence.length === 0 ? (
        ...
        <>
          {item.rectificationEvidence.some((e) => e.photo) ? (
            <div className="grid grid-cols-3 gap-2">
              {item.rectificationEvidence.map((e) => e.photo && <img key={e.id} src={e.photo} alt="" className="h-24 w-full rounded-lg object-cover" />)}
            </div>
  ...
  return (
    <SectionCard title="Inspection" icon={Search}>
      {item.inspectionHistory.length === 0 ? (
        ...
        <EventList events={item.inspectionHistory.slice().reverse().map((e, index) => ({ id: `${e.at}-${index}`, title: e.action, at: e.at, by: e.by, note: e.reason }))} dangerNote />
      )}
        ...
        {item.closeoutEvidence.map((e) => (
          <div key={e.id} className="rounded-xl border border-emerald-500/30 bg-emerald-500/5 p-3">
            {e.photo ? (
              ...
              <p className="rounded-lg border border-dashed border-border bg-background/50 p-3 text-xs text-muted-foreground">
                {item.type === "incomplete" ? "Completed without photo evidence" : "No photo attached"}
              </p>
            )}
...
export function EventList({ events, dangerNote }: { events: { id: string; title: string; at: string; by?: string; note?: string }[]; dangerNote?: boolean }) {
  return (
    <ul className="space-y-1 text-xs">
      {events.map((event) => (
        <li key={event.id} className="rounded-md border border-border/60 bg-background/40 px-2 py-1.5">
          <span className="font-medium capitalize">{event.title}</span>
          {event.by && <span className="text-muted-foreground"> · {event.by}</span>}
          <span className="text-muted-foreground"> · {new Date(event.at).toLocaleString()}</span>
          {event.note && <p className={`mt-0.5 ${dangerNote ? "text-destructive" : "text-muted-foreground"}`}>{event.note}</p>}
        </li>
      ))}
    </ul>
  );
